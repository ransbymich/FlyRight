...
class ClearanceViewTest(TestCase):
    ...
    def setUp(self):
        Clearance.objects.create(clearance_id=1, created_by='ray', message='valid', state='pending',
    ...
    def test_add_clearance(self):
        ...
        response = self.client.post(reverse('add clearance'), json.dumps(new_clearance),
    ...
    def test_get_clearance_by_clearance_id(self):
        ...
        response = self.client.post(reverse('get clearance by clearance_id'), json.dumps(clearance_id),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_clearance_by_created_by(self):
        ...
        response = self.client.post(reverse('get clearance by created by'), json.dumps(created_by),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 200)
        clearance = json.loads(response.content)
    ...
    def test_get_clearance_by_state(self):
        state = {"state": "pending"}
        response = self.client.post(reverse('get clearance by state'), json.dumps(state),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 200)
        clearance = json.loads(response.content)
        self.assertEqual(clearance[0]['clearance_id'], '1')
    ...
    def test_remove_clearance(self):
        clearance_id = {"clearance_id": 1}
        response = self.client.post(reverse('remove clearance'), json.dumps(clearance_id),
                                    content_type='application/json')
        self.assertEqual(response.status_code, 200)
